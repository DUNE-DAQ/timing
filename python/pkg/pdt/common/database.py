...
kSSPAddresses = {
    (0,5): range(0x20, 0x24), # APA1 ssp101 to 104
...
def __genRange( aNum, aGrp, aBaseName, aBaseId, aBaseTAddr, aFanout, aSlot ):
    return { '{}{}'.format(aBaseName, aBaseId+i+1):(aGrp, aBaseTAddr+i, aFanout, aSlot) for i in xrange(0,aNum) }
...
def findByTAddr( taddr ):
    uid = next( ( k for k,(_, ta, _, _) in __db__.iteritems() if taddr == ta ), None )
...
# ------------------------------------------------------------------------------
def findByGrp( aGrp ):
    return [ (k,)+(grp, taddr, fanout, slot) for k,(grp, taddr, fanout, slot) in __db__.iteritems() if grp == aGrp]
# ------------------------------------------------------------------------------
