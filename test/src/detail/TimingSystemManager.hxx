...
TimingSystemManager<MST_TOP, EPT_TOP>::TimingSystemManager(std::string cf)
  : TimingSystemManagerBase(cf)
{
...
template<class MST_TOP, class EPT_TOP>
const MST_TOP&
TimingSystemManager<MST_TOP, EPT_TOP>::getMaster(uint32_t aMstId) const
{
...
template<class MST_TOP, class EPT_TOP>
const EPT_TOP&
TimingSystemManager<MST_TOP, EPT_TOP>::getEndpoint(uint32_t endpoint_id) const
{
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::printSystemStatus() const
{
  std::cout << "Timing master status" << std::endl;
  for (uint32_t i = 0; i < getNumberOfMasters(); ++i)
    getMaster(i).get_status(true);
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::resetSystem() const
{
  for (uint32_t i = 0; i < getNumberOfMasters(); ++i)
    getMaster(i).reset();
  for (uint32_t i = 0; i < getNumberOfEndpoints(); ++i)
    getEndpoint(i).reset();
}
//-----------------------------------------------------------------------------
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::configureSystem() const
{
  for (uint32_t i = 0; i < getNumberOfMasters(); ++i)
    getMaster(i).configure();
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::resetPartition(uint32_t partition_id) const
{
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::configurePartition(uint32_t partition_id,
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::startPartition(uint32_t partition_id) const
{
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::stopPartition(uint32_t partition_id) const
{
...
template<class MST_TOP, class EPT_TOP>
uint64_t
TimingSystemManager<MST_TOP, EPT_TOP>::read_master_timestamp() const
{
...
template<class MST_TOP, class EPT_TOP>
uint64_t
TimingSystemManager<MST_TOP, EPT_TOP>::measure_endpoint_rtt(uint32_t address) const
{
...
template<class MST_TOP, class EPT_TOP>
uint64_t
TimingSystemManager<MST_TOP, EPT_TOP>::measure_endpoint_rtt(const ActiveEndpointConfig& ept_config) const
{
  ...
  if (lEptFanout < 0) {
    // TODO warning?
    TLOG() << "Endpoint config contains an fanout ept mux value. This is a system without fanouts";
  }
...
//-----------------------------------------------------------------------------
template<class MST_TOP, class EPT_TOP>
std::vector<EndpointRTTResult>
TimingSystemManager<MST_TOP, EPT_TOP>::performEndpointRTTScan()
{
  ...
  for (auto it = this->mExpectedEndpoints.begin(); it != this->mExpectedEndpoints.end(); ++it) {
    ...
    if (lRTT > this->mMaxMeasuredRTT)
      this->mMaxMeasuredRTT = lRTT;
...
template<class MST_TOP, class EPT_TOP>
void
TimingSystemManager<MST_TOP, EPT_TOP>::apply_endpoint_delays(uint32_t measure_rtt) const
{
  ...
  for (auto it = this->mExpectedEndpoints.begin(); it != this->mExpectedEndpoints.end(); ++it) {
    ...
    if (measure_rtt)
      lRttBefore = measure_endpoint_rtt(it->second);

    getMaster(0).get_master_node().apply_endpoint_delay(it->second, false);

    if (measure_rtt) {
      uint32_t lRttAfter = measure_endpoint_rtt(it->second);
      TLOG() << lEptIdD << " delay adjustment; cdel: " << lCDelay << ", fdel: " << lFDelay;
      TLOG() << "before RTT: " << std::dec << lRttBefore << ", after RTT: " << lRttAfter;
    }
  }
}
//-----------------------------------------------------------------------------
