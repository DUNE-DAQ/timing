...
ERS_DECLARE_ISSUE(timing,
                  EventReadError,
                  " Failed to read events from partition buffer. Requested: "
                    << std::to_string(red_number_events) << " Available: " << std::to_string(available_number_events),
...
{
  std::vector<uint32_t> accepted; // NOLINT(build/unsigned)
  std::vector<uint32_t> rejected; // NOLINT(build/unsigned)
};
...
public:
   ...
   */
  uint32_t read_trigger_mask() const; // NOLINT(build/unsigned)
   ...
   */
  uint32_t read_buffer_word_count() const; // NOLINT(build/unsigned)
   ...
   * @return     { description_of_the_return_value }
   */
  uint32_t num_events_in_buffer() const; // NOLINT(build/unsigned)
   ...
   */
  std::vector<uint32_t> read_events(size_t number_of_events = 0) const; // NOLINT(build/unsigned)
   ...
   */
  void start(uint32_t timeout = 5000) const; // NOLINT(build/unsigned)
   ...
   * @param[in]  timeout  in milliseconds
   */
  void stop(uint32_t timeout = 5000) const; // NOLINT(build/unsigned)
