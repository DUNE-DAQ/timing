...
//-----------------------------------------------------------------------------
void
GaiaDesign::configure(ClockSource clock_source, TimestampSource ts_source, TimestampEpoch epoch) const
{
  TopDesign::configure(clock_source);

  getNode<IRIGTimestampNode>("irig_time_source").set_ts_epoch(epoch);
